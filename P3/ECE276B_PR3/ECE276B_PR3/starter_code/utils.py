...
def car_next_error_state(time_step, cur_state, control, cur_ref_state, cur_iter, noise=True):
    # control term
    theta = cur_state[2]
    rot_3d_z = np.array([[np.cos(theta), 0], [np.sin(theta), 0], [0, 1]])
    f = rot_3d_z @ control
    ...
    # reference term
    next_ref_state = lissajous(cur_iter + 1)
    next_ref_state = np.array(next_ref_state)
    ref = cur_ref_state - next_ref_state

    if noise:
        return e + time_step * f.flatten() + ref + w
    else:
        return e + time_step * f.flatten() + ref
